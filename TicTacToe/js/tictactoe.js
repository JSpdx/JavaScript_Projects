//This function will get fired once the DO is leaded.
...
function rollForTurn() {
    ...
    for (i=0; i<xArray.length; i++) {
        ...
        setTimeout(function() {writeMsg(txt1);}, 1000); // time delay for dramatic effects 
    }
    // determine and concatenate tring showing which player won the roll
    if (pOne > pTwo) {
        first = 'Player 1';
...
//--------------------------------------------------------------------
// initiate the game, roll for turn & determine the acive player
//--------------------------------------------------------------------
function startGame() {
    var  xTurn = 0;
...
// this funtion will clear the text from the message console
function clearMsg() {
    document.getElementById('gameMsg').innerHTML = '';
}

//this function is for the player configuration panel and checks the
// proposedavatar assignments and prevents them from being the same.
...
function determineAvatar() {
    ...
    var avatarArray = getAvatars(); // returns an array of both player's assigned avatar
...
// as squares are selected theycheck in with this function to see if that particular
...
// and then concatentate the current move to it.
...
function check4Tie() {
    ...
    boardState = boardState.split(','); // separate the string by comma into an array
...
// whenever a win is detected the corresponding functin will 
...
function blink() {
    ...
    setTimeout(function() {body.style.backgroundColor = '#f79881';}, 800);
    setTimeout(function() {body.style.backgroundColor = '#c60343';}, 900);
...
function checkWinCon1(info,squareArray) {
    ...
    //this will rigger (ONLY) if there was a match for index0, index1, and index2
...
// checking fo wincon squares 147
...
function square1Animate() {
    ...
    if (activePlayer != "Game Stopped") { // if game has not yet started prevent avatar placement
        ...
        if (verdict == undefined) { // if verdict is empty than the square is unoccupied
            ...
            if (paintAvatar == 'O') { // change these all to ternary statmwnts instead
            ...
            avatarPlaced(square,paintAvatar); // end current turn and pass the turn to other player
